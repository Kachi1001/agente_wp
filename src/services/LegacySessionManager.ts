...
import type { ISessionManager, AdminSessionRow } from './types';
...
class LegacySessionManager implements ISessionManager {
  private sessions: Map<string, SessionData> = new Map();
  private authFolder: string = path.join(process.cwd(), 'auth_keys');
  private rebooting: Set<string> = new Set();
  // Contador de reboots consecutivos por sessão — alimenta o backoff exponencial.
  // Zerado em markConnected() quando a sessão estabiliza.
  private rebootAttempts: Map<string, number> = new Map();
  ...
  /** Promise de atraso simples — usado para timeouts e backoff. */
  private delay(ms: number): Promise<void> {
    return new Promise(r => setTimeout(r, ms));
  }

  /**
   * Remove os arquivos de lock do perfil do Chromium (SingletonLock/Cookie/Socket).
   *
   * Esses arquivos são a causa direta do erro "The browser is already running for
   * <profile>": o puppeteer recusa subir um navegador se o lock existe. Quando um
   * Chromium morre de forma abrupta (OOM, SIGKILL, crash do PM2) o lock fica órfão
   * e trava TODOS os boots seguintes. Limpamos antes de cada initialize().
   *
   * Seguro porque há exatamente um client por sessionId e o reboot sempre mata o
   * processo antes de relançar — nunca removemos o lock de um navegador vivo.
   */
  private clearSingletonLocks(sessionId: string): void {
    const sessionDir = path.join(this.authFolder, `session-${sessionId}`);
    for (const name of ['SingletonLock', 'SingletonCookie', 'SingletonSocket']) {
      try {
        // force:true ignora ENOENT; rmSync remove o symlink em si (no Linux o
        // SingletonLock é um symlink que pode estar pendurado/dangling).
        fs.rmSync(path.join(sessionDir, name), { force: true });
      } catch (err: any) {
        logger.warn(`[${sessionId}] Falha ao remover lock ${name}: ${err?.message}`);
      }
    }
  }
  ...
  async startSession(sessionId: string): Promise<void> {
    ...
    // Remove locks órfãos do perfil ANTES de subir o Chromium. Cobre tanto o
    // reboot (onde já matamos o processo) quanto o boot a frio após um crash do
    // PM2/SO, que deixa o SingletonLock pendurado e trava todo initialize().
    ...
    client.initialize().catch(err => {
      ...
      if (
        ...
        // O atraso até a próxima tentativa é gerido pelo backoff do rebootSession.
        this.rebooting.delete(sessionId);
        this.rebootSession(sessionId, `initialize error: ${msg}`);
      }
    });
  }
  ...
  /**
   * Snapshot para o painel /admin. No modo legado (monólito) não há processo
   * filho, então pid/uptime/restarts não se aplicam — preenchidos com neutros.
   */
  ...
  private markConnected(sessionId: string) {
    this.rebooting.delete(sessionId);
    // Sessão estabilizou — zera o backoff para que uma futura queda recomece
    // do atraso mínimo em vez de já partir do teto.
    this.rebootAttempts.delete(sessionId);
    const session = this.sessions.get(sessionId);
  ...
  private async killBrowser(client: Client): Promise<void> {
    ...
    // 0. Captura o PID do processo do Chromium ANTES de qualquer close — depois
    //    de close()/destroy() o browser.process() pode ficar indisponível, e é
    //    esse handle que garante o SIGKILL no final.
    let proc: any = null;
    try { proc = browser?.process?.() ?? null; } catch {}

    // 1. Tenta fechar a página graciosamente (com timeout — close() trava se o
    //    CDP já morreu).
    try {
      const page = (client as any).pupPage;
      if (page && !page.isClosed?.()) {
        await Promise.race([page.close().catch(() => {}), this.delay(3000)]);
      }
    } catch {}

    // 2. Fecha o browser (idem: timeout para não pendurar o reboot).
    try {
      if (browser) {
        await Promise.race([browser.close().catch(() => {}), this.delay(3000)]);
      }
    } catch {}

    // 3. destroy() da lib para limpar o estado interno.
    try {
      await Promise.race([client.destroy().catch(() => {}), this.delay(3000)]);
    } catch {}

    // 4. GARANTIA FINAL: mata o processo do SO via SIGKILL. Os close() acima são
    //    no-op quando a conexão CDP já estava morta — o processo seguiria vivo,
    //    segurando o SingletonLock e causando "browser is already running" no
    //    próximo boot. Este kill é o que de fato quebra o loop de navegador.
    try {
      if (proc?.pid && !proc.killed) {
        proc.kill('SIGKILL');
        logger.info(`[killBrowser] SIGKILL enviado ao Chromium (pid ${proc.pid}).`);
      }
    } catch (err: any) {
      logger.warn(`[killBrowser] Falha ao SIGKILL o Chromium: ${err?.message}`);
    }
  }
   ...
   */
  private async rebootSession(sessionId: string, reason: string) {
    ...
    // Backoff exponencial: 5s → 10s → 20s → 40s → 60s (teto). Sem isto, um erro
    // recorrente (ex.: "browser is already running") fazia o reboot rodar a cada
    // 3s indefinidamente, queimando CPU. markConnected() zera o contador.
    const attempts = (this.rebootAttempts.get(sessionId) ?? 0) + 1;
    this.rebootAttempts.set(sessionId, attempts);
    const backoffMs = Math.min(60000, 5000 * 2 ** Math.min(attempts - 1, 4));

    logger.warn(`[${sessionId}] Iniciando reboot da sessão (tentativa #${attempts}). Motivo: ${reason}. Religando em ${backoffMs}ms.`);
    ...
    // AGUARDA o kill terminar — sem isso o startSession corria contra um Chromium
    // ainda vivo e reincidia no "browser is already running". killBrowser tem
    // timeouts internos, então nunca pendura aqui.
    if (session) {
      await this.killBrowser(session.client).catch(() => {});
    }
    // Belt-and-suspenders: mesmo após o kill, remove qualquer lock remanescente.
    this.clearSingletonLocks(sessionId);
...
/**
 * Instância legada (monólito in-process). Usada apenas como fallback quando
 * WORKER_MODE=0. O caminho padrão é o Supervisor isolado (ver SessionManager.ts).
 */
